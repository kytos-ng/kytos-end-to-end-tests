...
if NOVISETTINGS:
    try:
        ...
        try:
            ...
            raise ValueError("Invalid NOVISETTINGS environment variable: must be a JSON encoded string or file")
elif NOVISWITCHES:
    switches = NOVISWITCHES.split()
    if len(switches) < 2:
        raise ValueError("Invalid NOVISWITCHES environment variable: must be a space-separated list of IPs")
    NOVISETTINGS = {}
    for i, sw in enumerate(switches, 1):
        NOVISETTINGS["vnovi%d" % i] = {"ip": sw}
elif os.environ.get("SWITCH_CLASS") == "NoviSwitch":
    raise ValueError("Neither NOVISWITCHES nor NOVISETTINGS environment variables found. You must define one of them")
...
if os.environ.get("SWITCH_CLASS") == "NoviSwitch" and (not NOVIUSER or not NOVIPASS):
    raise ValueError("Missing env vars for username/password for Noviflow switches: NOVIUSER and NOVIPASS")
...
if os.environ.get("SWITCH_CLASS") == "NoviSwitch" and not NOVILOCALIP:
    ...
    if not NOVILOCALIP:
        raise ValueError("Could not identify the Local IP for L2TP tunnels. Please define NOVILOCALIP env var or check pref ip source for switches")
...
class NoviSwitch( Switch ):
    ...
    def __init__( self, name, verbose=False, **kwargs ):
        ...
        self.ssh_client.connect(self.novi_ip, username=NOVIUSER, password=NOVIPASS, port=switch.get('ssh_port', 22))
        logger.debug("ssh_client connected sw=%s novisw=%s ip=%s" % (self.name, self.novi_name, self.novi_ip))
    ...
    def setup( cls ):
        ...
        for s, switch in NOVISETTINGS.items():
            ...
            client.connect(switch['ip'], username=NOVIUSER, password=NOVIPASS, port=switch.get('ssh_port', 22))
        ...
        if errors:
            logger.error('Found errors when checking for Noviflow switches:' + '\n'.join(errors))
            exit(1)
    ...
    def novi_start(self):
        ...
        for p in range(1, max_ifaces+1):
            ...
            if 'error' in result.lower():
                logger.error('error removing config from port %s in switch %s: %s\n' % (p, switch, result))
    ...
    def novi_setup_intf(self, intf1):
        ...
        intf2 = intf1.link.intf2 if (intf1.link.intf1.node.name == intf1.node.name and intf1.link.intf1.name == intf1.name) else intf1.link.intf1
    ...
    def get_l2tp_tun_id(self, intf1, intf2):
        link_name = "<->".join(sorted([f"{intf1.node.name}:{intf1.name}", f"{intf2.node.name}:{intf2.name}"]))
    ...
    def setup_link_noviflow(self, remote_ip, intf_name, l2tp_tun_id):
        ...
        logger.info("setup_link_noviflow port_num=%d l2tp_tun_id=%d\n" % (port_num, l2tp_tun_id))
    ...
    def setup_link_linux(self, remote_ip, intf_name, l2tp_port):
        ...
        udp_port = 17000 + l2tp_port
        cmd = 'ip l2tp add tunnel tunnel_id %s peer_tunnel_id %s remote %s local 0.0.0.0 encap udp udp_dport %d udp_sport %d 2>&1' % (l2tp_port, l2tp_port, remote_ip, udp_port, udp_port)
        result = run_cmd(cmd)
        if len(result) > 0:
            ...
            if "RTNETLINK answers: File exists" in result:
                ...
                result = run_cmd(cmd)
                if len(result) > 0:
                    logger.error('--> ERROR again: cmd=|%s| -- result=|%s|' % (cmd, result))
    ...
    def start( self, controllers ):
        ...
        for c in controllers:
            ...
            cmd = 'set config controller controllergroup c%d controllerid 1 priority 1 ipaddr %s port %d security none version of13' % (i, c_ip, c_port)
            result = self.novi_cmd(cmd)
            if 'error' in result.lower():
        ...
        result = self.novi_cmd(cmd)
        if 'error' in result.lower():
            log.error('error deleting ofserver config in switch %s: %s\n' % (self.novi_name, result))
    ...
    def novi_cmd(self, cmd):
        ...
        if not self.is_ssh_alive():
            self.ssh_client = paramiko.SSHClient()
            self.ssh_client.set_missing_host_key_policy(paramiko.AutoAddPolicy())
            self.ssh_client.connect(switch['ip'], username=NOVIUSER, password=NOVIPASS, port=switch.get('ssh_port', 22))
    ...
    def attach( self, intfName ):
        ...
        if not intf:
            logger.warning(f"attached unknown intf {intfName} into {self.name}/{self.novi_ip}! Ignoring...\n")
            return
        self.novi_setup_intf(intf)
    ...
    def dpctl( self, *args ):
        ...
        while time.time() - begin <= NOVIMAXWAIT:
            cmd = "ovs-ofctl dump-flows -O OpenFlow13 %s %s|grep -v OFPST_FLOW" % (switch, ' '.join(args[1:]))
            flows = run_cmd(cmd)
            if len(flows.splitlines()) == 0:
                break
            log.info(f"DEBUG: remaining flows for del-flows: {flows.splitlines()}")
            time.sleep(1)
    ...
    def reset_controller(self):
        ...
        result = self.novi_cmd(cmd)
        if 'error' in result.lower():
            log.error('error resetting controller in switch %s: %s\n' % (self.novi_name, result))
    ...
    def vsctl( self, *args ):
        ...
        if args[0] == "del-controller":
            ...
            for c in args[2:]:
                ...
                cmd = 'set config controller controllergroup c%d controllerid 1 priority 1 ipaddr %s port %s security none version of13' % (i, ip, port)
                result = self.novi_cmd(cmd)
                if 'error' in result.lower():
                    log.error('error running del-controller in switch %s: %s\n' % (self.novi_name, result))
        else:
